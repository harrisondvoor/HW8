...
my_connection = sqlite3.connect('tweets.db')
...
my_cursor.execute('DROP TABLE IF EXISTS Tweets')
my_cursor.execute('CREATE TABLE Tweets (tweet_id TEXT, author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweets NUMBER)')
...
for z in umsi_tweets:
    my_tuple = z['id'], z['user']['screen_name'], z['created_at'], z['text'], z['retweet_count']
    my_cursor.execute('INSERT INTO Tweets (tweet_id, author, time_posted, tweet_text, retweets) VALUES (?, ?, ?, ?, ?)', my_tuple)
...
my_cursor.execute('SELECT time_posted, tweet_text FROM Tweets')
...
# Print the results
my_cursor.execute('SELECT author FROM Tweets WHERE retweets>2')
more_than_2_rts = my_cursor.fetchall()
